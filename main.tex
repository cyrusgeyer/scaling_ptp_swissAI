...
\begin{figure}[H]
    ...
    random/.style={draw, rounded corners=1.5pt, minimum width=0.9cm, minimum height=0.5cm, inner sep=1.5pt, align=center, fill=green!10},
    ...
    junction/.style={circle, fill=black, inner sep=1.2pt}
    }
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \begin{tikzpicture}[font=\scriptsize, node distance=8mm]
            ...
            \node[call] (c1) {Model\\call \#1};
            \node[call, right=of c1] (c2) {Model\\call \#2};
            \node[call, right=of c2, faint] (c3) {Model\\call \#3};

            % Context directly above model call #1
            \node[token, fill=yellow!10, above=3mm of c1] (ctx) {context};
            \node[call, right=of c1] (c2) {Model\\call \#2};
            \node[call, right=of c2, faint] (c3) {Model\\call \#3};

            % Tokens below each call
            \node[token, below=8mm of c1] (t1) {$\text{token}_1$};
            \node[token, below=8mm of c2] (t2) {$\text{token}_2$};
            \node[token, below=8mm of c3, faint] (t3) {$\text{token}_3$};

            % Define junction points on the vertical arrows (midway between call and token)
            \coordinate (j1) at ($(c1)!0.5!(t1)$);
            \coordinate (j2) at ($(c2)!0.5!(t2)$);
            \coordinate (j3) at ($(c3)!0.5!(t3)$);

            % Random numbers positioned to the left, vertically centered between call and token
            \node[random, left=3mm of j1] (r1) {$u_1$};
            \node[random, left=3mm of j2] (r2) {$u_2$};
            \node[random, left=3mm of j3, faint] (r3) {$u_3$};

            % ===== BACKGROUND LAYER: All arrows =====
            \begin{scope}[on background layer]
                % Context to call #1: straight down
                \draw[arrow] (ctx.south) -- (c1.north);
                % Context to call #2: leave right center horizontally, enter top center vertically
                \draw[arrow] (ctx.east) .. controls +(right:8mm) and +(up:8mm) .. (c2.north);
                % Context to call #3: leave right center horizontally, enter top center vertically
                \draw[arrow,faint] (ctx.east) .. controls +(right:15mm) and +(up:8mm) .. (c3.north);
                ...
                % Lines from random numbers to junction points with circles
                \draw[line width=0.4pt] (r1.east) -- (j1);
                \draw[line width=0.4pt] (r2.east) -- (j2);
                \draw[line width=0.4pt, faint] (r3.east) -- (j3);

                % token_1 goes up and feeds into call #2 at left center (entering horizontally)
                \draw[arrow] (t1.east) .. controls +(right:8mm) and +(left:8mm) .. (c2.west);
                \draw[arrow,faint] (t1.east) .. controls +(right:14mm) and +(left:10mm) .. (c3.west);

                % token_2 goes up and feeds into call #3 at left center (entering horizontally)
                \draw[arrow,faint] (t2.east) .. controls +(right:8mm) and +(left:8mm) .. (c3.west);
            \end{scope}

            % Junction circles on top
            \node[junction] at (j1) {};
            \node[junction] at (j2) {};
            \node[junction, faint] at (j3) {};
        \end{tikzpicture}
        \par\vspace{1mm}
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \begin{tikzpicture}[font=\scriptsize, node distance=8mm]
            % Single model call
            \node[call] (pc) {Single\\model call};

            % Context directly above model call
            \node[token, fill=yellow!10, above=3mm of pc] (ctx) {context};

            % Random numbers to the left of model call
            \node[random, left=3mm of pc] (rall) {$u_1, u_2, u_3$};
            ...
            % Context to model: straight down
            \draw[arrow] (ctx.south) -- (pc.north);

            % Random numbers feed into model call from the left
            \draw[arrow] (rall.east) -- (pc.west);            % Model produces multiple tokens
            \draw[arrow] (pc) -- (pt1);
            \draw[arrow] (pc) -- (pt2);
            \draw[arrow] (pc) -- (pt3);
        \end{tikzpicture}
        \par\vspace{1mm}
        {\footnotesize\color{black!60}Parallel: multiple dependent tokens per call \citep{draxler2026parallel}.}
    \end{minipage}

    \caption{Autoregressive (AR) decoding requires sequential model calls where each token depends on prior context and tokens. Random variables $u_i$ represent the sampling procedure. PTP predicts multiple dependent tokens in a single model call by incorporating the auxiliaries $u_i$ into the model, enabling parallel generation without restrictive independence assumptions \citep{draxler2026parallel}.}
\end{figure}
\vspace{-2mm}
...
\textbf{Primary selling point:} PTP reduces the \emph{sequential} bottleneck of autoregressive decoding by predicting multiple dependent tokens per model call. In \citep{draxler2026parallel}, this is quantified via accepted tokens per verification step (higher implies fewer sequential steps). Our goal is to scale this approach to substantially larger models and provide reproducible evidence of latency reductions on long-form generation and agentic workflows.
